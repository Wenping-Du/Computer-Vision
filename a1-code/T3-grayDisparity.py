...
def random_search(offset, a, disparity, A, B, psize):
    ...
    while search_h > 1 and search_w > 1:
        ...
        if d < disparity[a_x, a_y] and b[0] < h and b[1] < w:
            disparity[a_x, a_y] = d
...
def patchmatch(img1, img2, psize, times):
    ...
    im = np.array(Image.open('/Users/deb/Documents/images/output/im3-1E.png'))
    # -----------initial offset and disparity ------------
    offset = np.ones_like(im, dtype=object) * np.nan
    ...
    for i in range(h):
        for j in range(w):
            ...
            y = offset[i, j][1]
            if x < h and y < w:
                newImage[i, j] = img2[x, y] - np.sqrt(disparity[i, j])
    return newImage


if __name__ == "__main__":
    array1 = np.array(Image.open('/Users/deb/Documents/images/im0_out.png').convert('L'))
    array2 = np.array(Image.open('/Users/deb/Documents/images/im1L_out.png').convert('L'))
    p_size = 1
    itr = 1
